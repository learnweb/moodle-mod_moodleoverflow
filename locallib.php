...
function moodleoverflow_print_latest_discussions($moodleoverflow, $cm, $page = -1, $perpage = 25) {
    ...
    $canmovetopic = has_capability('mod/moodleoverflow:movetopic', $context);
    ...
    $isloggedinuser = (!is_guest($context, $USER) && isloggedin());
    $cansubtodiscussion = ($isloggedinuser && $canviewdiscussions && subscriptions::is_subscribable($moodleoverflow, $context));
    ...
    $discussionpath = '/mod/moodleoverflow/discussion.php';

    // Check if the user can move topics to other moodleoverflows.
    $topicmovepossible = false;
    if ($isloggedinuser && $canmovetopic) {
        $coursemodules = get_fast_modinfo($moodleoverflow->course)->get_instances_of('moodleoverflow');
        $topicmovepossible = count(array_filter($coursemodules, fn($cm) => !$cm->deletioninprogress)) > 1;
    }
    ...
    $mustachedata = (object) [
        ...
        'cansubtodiscussion' => $cansubtodiscussion,
        'canmovetopic' => $topicmovepossible,
        'cannormoveorsub' => ((!$canmovetopic) && (!$cansubtodiscussion)),
 ...
 */
function moodleoverflow_print_discussion(
    $course,
    $cm,
    $moodleoverflow,
    $discussion,
    ...
    echo moodleoverflow_print_post(
        $post,
        $discussion,
        $moodleoverflow,
        $cm,
    ...
    echo moodleoverflow_print_posts_nested(
        $course,
        $cm,
        $moodleoverflow,
        $discussion,
...
function moodleoverflow_print_post(
    ...
    if (
        ($ownpost &&
        ($age < get_config('moodleoverflow', 'maxeditingtime')) &&
                    (!review::should_post_be_reviewed($post, $moodleoverflow) ||
                    !$post->reviewed)) ||
    ...
    if (
        ($ownpost && $notold && capabilities::has(capabilities::DELETE_OWN_POST, $modulecontext)) ||
    ...
    if (moodleoverflow_user_can_post($modulecontext, $post, false)) {
        ...
        if (empty($post->parent)) {
            ...
            if ($settingexist) {
                $infolimited = $limitedanswersetting->la_starttime ? " " . get_string(
                    'la_info_starttime',
                    'moodleoverflow',
                    ['limitedanswerdate' => date('d.m.Y H:i', $limitedanswersetting->la_starttime)]
                ) : '';
                $infolimited .= $limitedanswersetting->la_endtime ? " " . get_string(
                    'la_info_endtime',
                    'moodleoverflow',
    ...
    // Prepare the post.
    $post->message = file_rewrite_pluginfile_urls(
        $post->message,
        'pluginfile.php',
        $mcid,
        'mod_moodleoverflow',
...
function moodleoverflow_print_posts_nested(
    ...
    if (!empty($posts[$parent->id]->children)) {
        ...
        foreach ($posts as $post) {
            ...
            // Print its children.
            $output .= moodleoverflow_print_posts_nested(
                $course,
                $cm,
                $moodleoverflow,
                $discussion,
...
function moodleoverflow_add_new_post($post) {
    ...
    if (
        review::get_review_level($moodleoverflow) == review::EVERYTHING &&
